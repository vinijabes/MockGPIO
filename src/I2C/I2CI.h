...
#define CONFIG_REG_PGA_0_256V (0X0E00) // +/-0.256V range
...
{
public:
    I2CI(std::shared_ptr<GPIOI> gpio, GPIOPin clock, GPIOPin data)
        : m_GPIO(gpio), m_ClockPin(clock), m_DataPin(data)
    {
        gpio->SetPinMode(clock, GPIOMode::I2C);
        gpio->SetPinMode(data, GPIOMode::I2C);
    }
    virtual ~I2CI() {}

    virtual void SetSlaveAddress(unsigned char address) = 0;
    virtual double ReadVoltage(unsigned short channel) = 0;
    virtual int16_t ReadAnalogic(unsigned short channel) = 0;

    void SetConfig(unsigned int config) { m_Config = config; }
